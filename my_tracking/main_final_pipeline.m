% =========================================================================
% 终极雷达流水线: RELAX 旁瓣抑制 + Kalman时空防鬼影跟踪 + 杂波过滤与速度提取
% =========================================================================
...
for idx_frm = START_FRAME : TOTAL_FRAMES
    ...
    for iRg = 1 : nAdc
        if any(ShadowMask(iRg, :) == 1)
             valid_idx_sm = find(ShadowMask(iRg, :) == 1);
             [pw_subset, ~] = dbf(full_grid.angle(valid_idx_sm)', [], sorted_data_clean(:, :, iRg), antArray_Sorted.arrayPos, [], 'spacingCal', spacingCal);
             pwRA_Clean(iRg, valid_idx_sm) = pw_subset;
        end
    end
    
    %% 【Phase 2】: 桥接层 (CFAR 目标检测 + DBSCAN 点云聚类 + 多普勒测后提取)
    
    % === [手术刀 1] 全局绝对能量门限 ===
    GLOBAL_THRESHOLD_DB = -25; 
    max_energy = max(pwRA_Clean(:));
    if max_energy > 0
        energy_threshold = max_energy * (10^(GLOBAL_THRESHOLD_DB / 10));
        pwRA_Clean(pwRA_Clean < energy_threshold) = 0; 
    end
    % ===================================
    ...
    if ~isempty(cfar_iRange)
        ...
        if ~isempty(pc_coords)
            clusterRslt = pcCluster2D(pc_coords, 'pw', pc_pw, 'epsilon', p.epsilon, 'minpts', p.minpts, 'limitX', p.limitX, 'limitY', p.limitY);
    ...
    title(ax1, sprintf('Phase 1: RELAX Clean Heatmap (Frame: %d)', iFrm), 'FontSize', 12, 'FontWeight', 'bold');
    title(ax2, sprintf('Phase 2-4: CFAR & Target Tracking (Frame: %d)', iFrm), 'FontSize', 12, 'FontWeight', 'bold');
