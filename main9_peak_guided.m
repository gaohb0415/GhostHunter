% =========================================================================
% 峰值引导单目标雷达流水线: RELAX 车体抑制 + 全图Capon + ROI主峰局部检测 + 时序跟踪
% =========================================================================
...
hFig = figure('Name', 'Peak-Guided RELAX + Tracking', 'Position', [100, 100, 1200, 600]);
...
ax2 = subplot(1, 2, 2); hold on; axis equal; grid on; title('Phase 2-4: Peak-Guided Detection & Tracking');
xlim([-11, 11]); ylim([0, 12]); xlabel('X (m)'); ylabel('Y (m)');
...
for idx_frm = START_FRAME : TOTAL_FRAMES
    ...
    for iRg = 1 : size(pwRA_Clean, 1)
        roi_idx = find(ShadowMask(iRg, :) == 1);
        if isempty(roi_idx)
            continue;
        end
        roi_row = pwRA_Clean(iRg, roi_idx);
        bg_val = median(roi_row);
        roi_row_enh = roi_row - bg_val;
        roi_row_enh(roi_row_enh < 0) = 0;
        roi_row_enh = roi_row_enh .^ 1.2;
        pwRA_Proc(iRg, roi_idx) = roi_row_enh;
    end

    [cfar_iRange, cfar_iAngle, ~] = cfar2D(pwRA_Proc, cfarParamRA);
    pc_coords = [];
    pc_pw = [];
    pc_R = [];
    pc_A = [];
    ...
    if ~isempty(cfar_iRange)
        roi_map = pwRA_Proc;
        roi_map(ShadowMask ~= 1) = 0;
        roi_vals = roi_map(ShadowMask == 1);
        ...
        if isempty(roi_vals) || max(roi_vals(:)) <= 0
            ...
            for k = 1:length(cfar_iRange)
                ...
                inROI = ShadowMask(r_idx, a_idx) == 1;
                inPeakGate = abs(curr_R - peak_R) <= PEAK_GATE_R && abs(curr_A - peak_A) <= PEAK_GATE_A;
            ...
            if ~isempty(cfar_iRange)
                ...
                if ~isempty(pc_coords)
                    ...
                    clusterRslt = pcCluster2D(pc_coords, 'pw', pc_pw, 'epsilon', p.epsilon, 'minpts', 1, 'limitX', p.limitX, 'limitY', p.limitY);
                    clusterRslt = keepStrongestCluster(clusterRslt);
    ...
    if iFrm == START_FRAME
        ...
        if structLength(clusters(iFrm).cluster, 'centroid') > 0
            ...
            clusterNew1 = struct('centroid', zeros(0,2), 'ghostLabel', zeros(0,1), 'velocity', zeros(0,1), 'pc', {{}});
        end
        ...
        % 先尝试用 lost 轨迹重激活，而不是直接进 candidate
    ...
    title(ax2, sprintf('Phase 2-4: Peak-Guided Detection & Tracking (Frame: %d)', iFrm), 'FontSize', 12, 'FontWeight', 'bold');
    ...
    if ~isempty(pc_coords)
        scatter(ax2, pc_coords(:,1), pc_coords(:,2), 12, [0.7 0.7 0.7], 'filled');
    end
...
empty_cluster(1) = [];
rslt = struct('cluster', empty_cluster, 'noise', struct('pc', []), 'pcInput', [], 'pw', [], 'clusterIdx', []);
end

function clusterRslt = keepStrongestCluster(clusterRslt)
if structLength(clusterRslt.cluster, 'centroid') <= 1
    return;
end

pcInputAll = clusterRslt.pcInput;
pwAll = clusterRslt.pw;
clusterIdxAll = clusterRslt.clusterIdx;
score = -inf(structLength(clusterRslt.cluster, 'centroid'), 1);
for c_idx = 1:length(clusterRslt.cluster)
    idx = clusterIdxAll == c_idx;
    if any(idx)
        score(c_idx) = sum(pwAll(idx));
    end
end
[~, bestIdx] = max(score);
keepIdx = clusterIdxAll == bestIdx;
clusterRslt.cluster = clusterRslt.cluster(bestIdx);
clusterRslt.noise.pc = pcInputAll(~keepIdx, :);
clusterRslt.pcInput = pcInputAll(keepIdx, :);
clusterRslt.pw = pwAll(keepIdx);
clusterRslt.clusterIdx = ones(sum(keepIdx), 1);
end
