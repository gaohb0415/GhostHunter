...
% see also pcolor
% 加入了字体设置、点云绘制 刘涵凯 更新: 2022-7-5
% 加入了spoke和circle的隐藏 刘涵凯 更新: 2023-6-6
%
%%  InputParseer
p = inputParser();
...
p.addOptional('pcPlot', 0);
p.addOptional('pointCloud', []);
p.addOptional('textEn', 1);
p.addOptional('textFontSize', 15);
p.addOptional('axisEn', 1); % 是否绘制spoke和circle
p.parse(varargin{:});
...
pcPlot = p.Results.pcPlot;
pointCloud = p.Results.pointCloud; % N*2矩阵, N为点云数, 2为距离, 角度
textEn = p.Results.textEn;
textFontSize = p.Results.textFontSize;
axisEn = p.Results.axisEn;
...
%% 绘制点云 lhk
% 对坐标进行了"中心"处理, 使点云位于热度框的中心而不是顶点
if pcPlot
    if any(pointCloud)
        rNormCloudPoint = pointCloud(:, 1) * rNorm(end) + (rNorm(2) - rNorm(1)) / 2;
        pointCloudX = rNormCloudPoint .* sind(pointCloud(:, 2));
        pointCloudY = rNormCloudPoint .* cosd(pointCloud(:, 2));
        h2 = plot(pointCloudX, pointCloudY, '.', 'Color', 'r');
    end
end
...
%     caxis([quantile(Z(:),0.01),quantile(Z(:),0.99)])
    ...
    c = [];
    % caxis([quantile(Z(:),0.01),quantile(Z(:),0.99)]) % 添加此句使其在不加bar时也保持此数据范围 lhk
end
%% Outputs
nargoutchk(0,2)
if nargout==1
                ...
                if and(thetaMin==0,thetaMax == 360),
                    if spokeMesh(kk)<360,
                        text(1.05.*contourD(end).*cost(kk),...
                            ...
                            'FontSize', textFontSize, 'FontName', textFontName, ...
                            'horiz', 'center', 'vert', 'middle');
                    end
                else
                    % 原为1.05
                    t = text(1.07.*contourD(end).*cost(kk),...
                        1.07.*contourD(end).*sint(kk),...
                        [num2str(spokeMesh(kk),3),char(176)],...
                        'FontSize', textFontSize, 'FontName', textFontName, ...
                        'horiz', 'center', 'vert', 'middle');
                    % 防止-90和90遮挡距离栏
                    if (spokeMesh(kk) > -92 && spokeMesh(kk) < -87)
                        t.Position =  t.Position + [-0.015,0.05,0];
                    end
                    if (spokeMesh(kk) < 92 && spokeMesh(kk) > 87)
                        t.Position =  t.Position + [-0.005,0.05,0];
                    end
                    if (spokeMesh(kk) > -5 && spokeMesh(kk) < 5)
                        t.Position =  t.Position + [0,-0.03,0];
                    end
                end
            end
        end
    ...
    function createCircles(rMin,rMax,thetaMin,thetaMax,Ncircles,circlePos,Nspokes)
        ...
        for kk=1:Ncircles
            if isempty(RtickLabel)
                ...
                rtick = RtickLabel{kk}; % 2024-1-5修改()为{} lhk
            end
            ...
            if textEn
                t = text(contourD(kk).*cosd(position),...
                    ...
                    [rtick, 'm'], 'FontSize', textFontSize, 'FontName', textFontName, ...
                ...
                elseif max(round(theta))==90 && strcmpi(typeRose,'meteo')
                    % 原为[0,0.05,0]
                    if kk == Ncircles
                        % t.Position =  t.Position - [0,0.07,0];
                        t.Position =  t.Position - [-0.1,0.07,0];
                    else
                        t.Position =  t.Position - [-0.1,0.07,0];
                    end
                    % t.Interpreter = 'latex';
            ...
            if max(theta)-min(theta)>180,
                t = text(contourD(end).*1.3.*cosd(position),...
                    ...
                    [labelR], 'FontSize', textFontSize, 'FontName', textFontName, ...
