...
% 根据https://ww2.mathworks.cn/matlabcentral/fileexchange/16581-hidden-point-removal修改
[numPts, dim] = size(p);
p = p - C; %Move C to the origin
normp = sqrt (sum (p.^2, 2)); %Calculate ||p||
R = max(normp)*(10^param); %Sphere radius
p = p + 2*(R-normp).*(p./normp); %Spherical flipping
visiblePtInds = convhull ([p; zeros(1,dim)]); %convex hull
visiblePtInds(visiblePtInds==numPts+1)=[];
