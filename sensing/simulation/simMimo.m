...
% for iFrm = [60]
    ...
    %% 多径反射
    [ptGhostNow, amplGhost] = multipath(vert(:, :, iFrm), ct(:, :, iFrm), squeeze(joint(1, :, iFrm))); % 鬼影及信号幅度
    ptGhostNext = ptGhostNow +  mean(ptNext - ptNow, 1); % 简单起见, 直接给鬼影赋予本体的位移

    %% 帧间坐标插值
    ptInterp = ptChirpInterp([ptNow; ptGhostNow], [ptNext; ptGhostNext]);
    % ptInterp = ptChirpInterp(ptNow, ptNext);

    ptInterp = ptInterp(1:100, :, :);
    ampl = ampl(1:100,:,:);
    % 信号合成
    % simSignal = synthFromPt(ptInterp, [ampl;  p.amplGhostFac * amplGhost]);
    tic
    simSignal = synthFromPt(ptInterp, ampl);
    toc

    %% 加入空采环境信号
    hEmpty = 'G:\radarData\23.11.20\empty\48chn\10s\24chn\'; % 整理后的空采数据地址
    load([hEmpty, num2str(randi(4100)), '.mat'])
    radarData = simSignal + empty24Chn;

    %% 保存数据
    % hSave = [p.handleSimSignal, '\mimo\', num2str(iFrm), '.mat'];
    % save(hSave, 'radarData')

    %% RAM展示
    [pwRA, ~] = dbfProc1D(fftRange(radarData - mean(radarData, 2)), 'sigReconsEn', 0, 'limitR', [0, 8], 'resAng', 1, 'drawEn', 1); drawnow;
